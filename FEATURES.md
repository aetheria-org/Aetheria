...
- **Capes** — Mod-wide custom cape support.
...
- **Custom Scoreboard** — Fully configurable scoreboard replacement with per-line text alignment, minimum width slider, and powder line display.
## Storage

- **Storage Overlay** — Custom overlay for viewing all storage containers with search, scrollbar, multiple display styles, dim-based slot highlighting, and smooth container switching. Use `/storage <n>` to jump to a specific container.
## Waypoints
...
- **Dungeon Overlay** — Shows run timers, end stats, and Scarf & Professor splits.
...
+ `/jef reload` — Reloads repo data and lists item tags.
...
+ `/storage <n>` — Opens storage overlay and jumps to container number.
