...
import io.hamlook.aetheria.features.profile.data.base.NetworthData;
import io.hamlook.aetheria.features.profile.data.base.Statistics;
...
import io.hamlook.aetheria.features.profile.data.slayer.Slayer;
import io.hamlook.aetheria.features.profile.data.slayer.SlayerData;
...
public class ProfileParser {
    ...
    public static void save(){
        ...
        writeToJson(profile);
        parsing = false;
        Aetheria.logger.info("[ProfileParser] Saved profile: " + base.playerName);
    ...
    public static void parse(String player, Container container) {
        ...
        GuiWaiter.waitFor("View Inventory",2,8,"View Profile",inv -> {
            ...
            if (inventory[0] == null) {
                Aetheria.logger.info("[ProfileParser] InventoryData was null for: " + base.playerName);
                parsing = false;
                return;
            }
            ...
            GuiWaiter.waitFor("View Skills",2,49,"View Profile",skills -> {
                ...
                if(skill[0] == null){
                    Aetheria.logger.info("[ProfileParser] SkillData was null for: " + base.playerName);
                    parsing = false;
                    return;
                }
                ...
                GuiWaiter.waitFor("View HOTM", 2,31,"View Profile",hotm -> {
                    ...
                    if(mountain[0] == null){
                        Aetheria.logger.info("[ProfileParser] HOTMData was null for: " + base.playerName);
                        parsing = false;
                        return;
                    }
                    ...
                    GuiWaiter.waitFor("View Dungeon Stats",2,49,"View Profile",dungeon -> {
                        ...
                        if(dungeonData[0] == null){
                            Aetheria.logger.info("[ProfileParser] DungeonData was null for: " + base.playerName);
                            parsing = false;
                            return;
                        }
                        Aetheria.logger.info("[ProfileParser] DungeonData parsed for: " + base.playerName);
                    },prof4 -> {
                        ...
                        GuiWaiter.waitFor("View Slayers",2,31,"View Profile", slayers -> {
                            ...
                            if(slayerData[0] == null){
                                Aetheria.logger.info("[ProfileParser] SlayersData was null for: " + base.playerName);
                                parsing = false;
                                return;
                            }
                            ...
                            GuiWaiter.waitForPaged("View Wardrobe", 2, 53, "Next Page", 48, "View Profile", chest -> wardrobe.putAll(parseWardrobe(chest)),
                                    prof6 -> {
                                        Aetheria.logger.info("[ProfileParser] WardrobeData parsed for: " + base.playerName);
                                        ...
                                        GuiWaiter.waitForPaged("View Pets",2,53,"Next Page",49,"View Profile",
                                                ...
                                                prof7 -> {
                                                    Aetheria.logger.info("[ProfileParser] PetsData parsed for: " + base.playerName);
    ...
    private static void parseNext(int index, List<Integer> slotsToCheck, List<ContainerData> containers, int currentWindowId) {
        if (index >= slotsToCheck.size()) {
            if (containers.isEmpty()) {
                Aetheria.logger.info("[ProfileParser] StorageData was null for: " + base.playerName);
                parsing = false;
                return;
            }
        ...
        GuiWaiter.waitFor("Show Contents", 2, -6, "View Storage",
                storage -> {
                    if (storage == null) {
                        Aetheria.logger.info("Empty Container: " + id);
    ...
    public static SlayersData parseSlayer(ContainerChest container) {
        ...
        for(Slayer slayer : Slayer.values()){
            ...
            if(stack == null) { Aetheria.logger.info("[Slayer] Null stack at slot " + slayer.itemSlot + " for " + slayer + " — skipping"); continue; }
            if(!ColorUtils.stripColor(stack.getDisplayName()).equals(slayer.itemName)) {
                Aetheria.logger.info("[Slayer] Name mismatch for " + slayer + " at slot " + slayer.itemSlot + " — got: " + ColorUtils.stripColor(stack.getDisplayName()) + ", expected: " + slayer.itemName + " — skipping");
                continue;
            }
            List<String> lore = getLore(stack);
            ...
            try {
                ...
                if(curLevel < 0) { Aetheria.logger.info("[Slayer] No level found for " + slayer + " — skipping"); continue; }
                Aetheria.logger.info("[Slayer2] " + slayer + " | " + slayers.size());
                slayers.put(slayer,new SlayerData(
                        curLevel,curExp,reqExp,t1Kills,t2Kills,t3Kills,t4Kills,t5Kills,drops
                ));
                Aetheria.logger.info("[Slayer3] " + slayer + " | " + slayers.size());
            }catch (NumberFormatException e){
                Aetheria.logger.info(e.getMessage());
            }
        }
    ...
    public static FloorData parseFloor(ItemStack stack){
        ...
        if(floor == Floor.FLOOR_SEVEN){
            Aetheria.logger.info("Parsing Floor Seven");
        }
        ...
        if(floor == Floor.FLOOR_SEVEN){
            Aetheria.logger.info("Tried Parsing Floor Seven");
        }
        if(bossKills < 0){
            if(floor == Floor.FLOOR_SEVEN){
                Aetheria.logger.info("Error Parsing Floor Seven");
    ...
    public static HOTMData parseHOTM(ContainerChest container) {
        ...
        for(String s : lore){
            ...
            if (s.contains("/") && !s.contains(" ")) {
                try {
                    String[] parts = s.split("/");
                    Aetheria.logger.info("[ProfileParser] Parsing " + parts[0] + " " + parts[1]);
        ...
        if(tier < 0 || tokens < 0 || mithril < 0 || gemstone < 0 || commisions < 0){
            Aetheria.logger.info("Tier: " + tier + " | Tokens: " + tokens + " | Mithril: " + mithril + " | Gemstone: " + gemstone + " | Comms: " + commisions);
            return null;
        }
    ...
    public static ItemStack itemFromNBT(String data) {
        try {
            ...
            Aetheria.logger.info("Error parsing item from NBT: " + data);
            return null;
        }
    }
        ...
        for (String line : getLore(stack)) {
            if (line.startsWith("Profile:")) {
                ...
                if (words.length >= 2) {
                    ...
                    Aetheria.logger.info("Profile: " + lastCachedProfile);
                }
            }
        }
    }
    ...
    public static SkillsData parseSkills(ContainerChest container) {
        ...
        for(Skill skill : Skill.values()){
            ...
            if(!skill.name.equalsIgnoreCase(words[0])) {
                Aetheria.logger.info("Wrong Slot for Skill: " + skill.name + " | " + skill.slot + " | " + words[0]);
                continue;
            }
            List<String> lore = getLore(stack);
            ...
            try{
                ...
                Aetheria.logger.info(skill.name() + " Level: " + currentLevel);
    ...
    public static void writeToJson(ProfileData data) {
        if (data == null) return;
        File file = new File(ATHRConfig.configDirectory, "profile.bin");
        if (!file.exists()) {
            try { file.createNewFile(); }
            catch (IOException e) { Aetheria.logger.info("Error creating profile.bin"); return; }
        }
        ...
        File file1 = new File(ATHRConfig.configDirectory, "profile.json");
        if (!file1.exists()) {
