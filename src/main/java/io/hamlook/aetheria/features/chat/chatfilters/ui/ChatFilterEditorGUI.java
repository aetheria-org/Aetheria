...
public class ChatFilterEditorGUI extends ChatFilterBaseGUI {
    ...
    private int lx() {
        return boxX + getScaledX(PAD);
    }

    private int rx() {
        return boxX + getScaledX(SPLIT);
    }

    private int rw() {
        return boxW - getScaledX(SPLIT) - getScaledX(PAD);
    }

    private int secMatchY() {
        return boxY + getScaledY(PAD + HEADER_H + ROW_GAP);
    }

    private int secCaseY() {
        return secMatchY() + getScaledY(LABEL_H + ROW_GAP + TOGGLE_H + SEC_GAP);
    }

    private int secActionY() {
        return secCaseY() + getScaledY(LABEL_H + ROW_GAP + TOGGLE_H + SEC_GAP);
    }

    private int secCustomY() {
        return secActionY() + getScaledY(LABEL_H + ROW_GAP + TOGGLE_H + SEC_GAP);
    }

    private int secPreviewY() {
        return secCustomY() + getScaledY(LABEL_H + ROW_GAP + FIELD_H + (customPreviewResult.isEmpty() ? 0 : LABEL_H + 4) + ROW_GAP);
    }

    private int bottomBtnY() {
        return boxY + boxH - getScaledY(PAD + BTN_H);
    }
    ...
    private String buildPreviewExample(String word, int index) {
        ...
        switch (mode) {
            case STARTS:
                return "§e" + word + "§r quick brown fox";
            case ENDS:
                return "quick brown fox §e" + word + "§r";
            default:
                return "quick §e" + word + "§r brown fox";
        }
    }
    ...
    protected void actionPerformed(GuiButton button) {
        switch (button.id) {
            case 0:
                ...
                break;
            case 1:
                mode = FilterMode.STARTS;
                break;
            case 2:
                mode = FilterMode.ENDS;
                break;
            case 3:
                mode = FilterMode.CONTAINS;
                break;
            case 4:
                filterCase = FilterCase.SENSITIVE;
                break;
            case 5:
                filterCase = FilterCase.INSENSITIVE;
                break;
            case 6:
    ...
    protected void onKeyTyped(char typedChar, int keyCode) {
        if (keyCode == 1) {
            ...
            if (customPreviewField.isFocused()) {
    ...
    private void drawSectionBackgrounds(ListLayout left, ListLayout right) {
        ...
        // Left: word list area
        NineSliceUtils.draw(Resources.storageBackground(1), left.x - pad, left.y - pad, left.width + pad * 2, left.height + pad * 2, 6, 18);
        ...
        NineSliceUtils.draw(Resources.storageBackground(1), right.x - pad, settingsTop, right.width + pad * 2, settingsBot - settingsTop, 6, 18);

        // Right: preview section
        NineSliceUtils.draw(Resources.storageBackground(1), right.x - pad, settingsBot, right.width + pad * 2, (bottomBtnY() - getScaledY(4)) - settingsBot, 6, 18);
    ...
    private void drawHeader() {
        GlStateManagerCompat.color(1f, 1f, 1f, 1f);
        TextRenderUtils.drawStringScaleAware(isNew ? "§lCreate Chat Filter" : "§lEdit Chat Filter", lx(), boxY + getScaledY(PAD), textScale * 1.4f, false);
    ...
    private void drawSettingsPanel() {
        int sx = rx();
        GlStateManagerCompat.color(1f, 0.8f, 0.2f, 1f);
        ...
        GlStateManagerCompat.color(0.75f, 0.75f, 0.75f, 1f);
    ...
    private void drawLeftWordList(int mouseX, int mouseY, ListLayout layout) {
        if (words.isEmpty()) {
            GlStateManagerCompat.color(0.45f, 0.45f, 0.45f, 1f);
