...
public class ProfileViewerAPI {
            ...
            .registerTypeHierarchyAdapter(EnumMap.class, (JsonDeserializer<EnumMap<?, ?>>) (json, typeOfT, context) -> {
                ...
                for (Map.Entry<String, JsonElement> entry : json.getAsJsonObject().entrySet()) {
                    try {
                        ...
                        JefMod.logger.info("Skipped unknown enum key from API: " + entry.getKey());
                    }
                }
                return map;
            })
    ...
    private static final ExecutorService networkExecutor = Executors.newSingleThreadExecutor(r -> {
        Thread t = new Thread(r, "JEF-ProfileViewerAPI");
        t.setDaemon(true);
        return t;
    });
    ...
    public static void fetchFromAPI(String username){
        ...
        networkExecutor.execute(() -> {
            try{
                ...
                if(profile == null){
                    JefMod.logger.info("Null Profile");
                }
                profileHashMap.put(username,profile);
                lastFetches.put(username,System.currentTimeMillis());
                JefMod.logger.info("Added " + username + " to profile list.");
            } catch (Exception e) {
                JefMod.logger.info("Error While Fetching Profiles For: " + username);
                e.printStackTrace();
    ...
    public static PlayerProfile fetchUser(String username) throws Exception{
        ...
        conn.setConnectTimeout(5000);
        conn.setReadTimeout(5000);
        JefMod.logger.info("Code: " + conn.getResponseCode());

        if (conn.getResponseCode() == 200) {
            String json = readResponse(conn);
            File file = new File(JefConfig.configDirectory, "test_profiles.json");
            if(!file.exists()){
                file.createNewFile();
            }
